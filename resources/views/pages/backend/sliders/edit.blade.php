...
@section('title', __('Slider Düzenleme'))
...
<div class="page-title d-flex flex-column justify-content-center flex-wrap me-3 py-5">
    <h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center my-0">{{ __('Slider Düzenleme') }}</h1>
...
<div id="kt_app_content" class="app-content flex-column-fluid">
    <div id="kt_app_content_container" class="app-container container-fluid">
        ...
        @php
            $sliderImageSize = auth()->user()->client?->getImageSize('slider');
        @endphp
        @if($sliderImageSize)
            <div class="alert alert-dismissible bg-light-warning border border-warning border-dashed d-flex flex-column flex-sm-row p-5 mb-4">
                ...
                <div class="d-flex flex-column text-warning">
                    ...
                    <span>- Yükleyeceğiniz slider resmi, belirlenen en boy oranına (<strong>{{ $sliderImageSize['width'] }}x{{ $sliderImageSize['height'] }} px</strong>) göre sistem tarafından otomatik olarak kesilecektir (crop).</span>
                </div>
            </div>
        @endif
        ...
        <div class="card card-flush mb-5 mb-xl-10">
            <div class="card-header align-items-center gap-2 gap-md-5 position-relative ribbon ribbon-start min-h-60px">
                <div class="ribbon-label bg-warning fs-6" style="padding: 10px 15px;">
                    <span class="d-flex text-white fw-bolder fs-6">{{ __('Slider Bilgileri') }}</span>
                </div>
            ...
            <div class="card-body pt-6">
                <form action="{{ route($routePrefix . 'sliders.update', $slider->id) }}" method="POST" id="kt_sliders_edit_form" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="col-md-12 mb-6">
                            <div class="tab-content">
                                @foreach($clientLanguages as $lang)
                                @php
                                    $titleVal = $slider->getTranslation('title', $lang->code, false);
                                    $descVal = $slider->getTranslation('description', $lang->code, false);
                                @endphp
                                <div class="tab-pane fade {{ $activeTabCode === $lang->code ? 'show active' : '' }}" id="lang_tab_{{ $lang->code }}" role="tabpanel">
                                    <div class="mb-6">
                                        <label class="form-label {{ $lang->is_default ? 'required' : '' }}">{{ __('Slider Başlığı') }} ({{ strtoupper($lang->code) }})</label>
                                        <input type="text" name="title[{{ $lang->code }}]" class="form-control form-control-solid @error('title.'.$lang->code) border-danger @enderror" placeholder="{{ __('Slider Başlığı Giriniz') }}" value="{{ old('title.'.$lang->code, $titleVal) }}" />
                                        @error('title.'.$lang->code)
                                            <div class="text-danger small mt-1">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    <div class="mb-0">
                                        <label class="form-label">{{ __('Slider Açıklaması') }} ({{ strtoupper($lang->code) }})</label>
                                        <input type="text" name="description[{{ $lang->code }}]" class="form-control form-control-solid @error('description.'.$lang->code) border-danger @enderror" placeholder="{{ __('Slider Açıklaması Giriniz') }}" value="{{ old('description.'.$lang->code, $descVal) }}" />
                        ...
                        <div class="col-md-6 mb-0">
                            ...
                            <select name="status" class="form-select form-select-solid @error('status') border-danger @enderror" data-control="select2" data-hide-search="true" data-placeholder="{{ __('Seçim Yapınız') }}">
                                <option value=""></option>
                                <option value="active" {{ old('status', $slider->status) == 'active' ? 'selected' : '' }}>{{ __('Aktif') }}</option>
                                <option value="passive" {{ old('status', $slider->status) == 'passive' ? 'selected' : '' }}>{{ __('Pasif') }}</option>
                            </select>
                            @error('status')
                                <div class="text-danger small mt-1">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-6 mb-0">
                            <label class="form-label">
                                {{ __('Slider Resmi') }}
                                @if($slider->image)
                                    <a data-fslightbox="slider-edit-gallery" data-type="image" href="{{ asset('storage/' . $slider->image) }}" class="text-primary fw-bold ms-2">({{ __('Mevcut Resim') }})</a>
                                @endif
                            </label>
                    ...
                    <div class="d-flex justify-content-start">
                        <button type="submit" id="kt_sliders_edit_submit" class="btn btn-sm btn-warning me-3">
                            <span class="indicator-label">{{ __('Değişiklikleri Kaydet') }}</span>
                            <span class="indicator-progress">{{ __('Lütfen bekleyin...') }} 
                            <span class="spinner-border spinner-border-sm align-middle ms-2"></span></span>
                        </button>
                        <a href="{{ route($routePrefix . 'sliders.index') }}" class="btn btn-sm btn-light btn-active-light-primary">{{ __('Geri Dön') }}</a>
                    </div>
                </form>
            </div>
        </div>
...
<script>
    const form = document.getElementById('kt_sliders_edit_form');
    const submitButton = document.getElementById('kt_sliders_edit_submit');
