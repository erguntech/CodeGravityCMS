...
@section('title', __('Ürün Kategorilerini Sırala'))
...
<div class="page-title d-flex flex-column justify-content-center flex-wrap me-3 py-5">
    <h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center my-0">{{ __('Ürün Kategorilerini Sırala') }}</h1>
        ...
        <div class="card card-flush mb-5 mb-xl-10">
            <div class="card-header align-items-center gap-2 gap-md-5 position-relative ribbon ribbon-start min-h-60px">
                ...
                <div class="card-toolbar">
                    <a href="{{ route($routePrefix . 'product-categories.index') }}" class="btn btn-sm btn-light btn-active-light-primary" style="border-radius: 4px;">{{ __('Geri Dön') }}</a>
                </div>
            </div>
...
<script>
    $(document).ready(function() {
        var updateOrder = function() {
            ...
            $.ajax({
                url: "{{ route($routePrefix . 'product-categories.reorder.update') }}",
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    order: order
