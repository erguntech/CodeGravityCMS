...
@section('title', __('Hizmetleri Sırala'))
...
<div class="page-title d-flex flex-column justify-content-center flex-wrap me-3 py-5">
    <h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center my-0">{{ __('Hizmetleri Sırala') }}</h1>
        ...
        @if(!$services->isEmpty())
            ...
            <div class="alert alert-dismissible bg-light-warning border border-warning border-dashed d-flex flex-column flex-sm-row p-5 mb-4">
                ...
                <div class="d-flex flex-column text-warning">
                    ...
                    <span>- Hizmetlerin sol tarafındaki taşıma ikonunu (handle) kullanarak sürükle-bırak yöntemiyle sıralamalarını değiştirebilirsiniz.</span>
        ...
        <div class="card card-flush mb-5 mb-xl-10">
            <div class="card-header align-items-center gap-2 gap-md-5 position-relative ribbon ribbon-start min-h-60px">
                <div class="ribbon-label bg-warning fs-6" style="padding: 10px 15px;">
                    <span class="d-flex text-white fw-bolder fs-6">{{ __('Hizmet Sıralama Bilgileri') }}</span>
                </div>
                <div class="card-title"></div>
                <div class="card-toolbar">
                    <a href="{{ route($routePrefix . 'services.index') }}" class="btn btn-sm btn-light btn-active-light-primary" style="border-radius: 4px;">{{ __('Geri Dön') }}</a>
                </div>
            </div>
                ...
                @if($services->isEmpty())
                    ...
                    <div class="alert alert-dismissible bg-light-danger border border-danger border-dashed d-flex flex-column flex-sm-row p-5 mb-0">
                        ...
                        <div class="d-flex flex-column text-danger">
                            ...
                            <span>- Sıralanacak herhangi bir hizmet bulunamadı.</span>
                        </div>
                    </div>
                @else

                    <!-- Sortable Vertical List -->
                    <div class="d-flex flex-column gap-3 mb-0" id="kt_services_sortable">
                        @foreach($services as $service)
                            <div class="d-flex align-items-center justify-content-between border border-gray-300 rounded p-4 bg-light" data-id="{{ $service->id }}">
                                <div class="d-flex align-items-center gap-4">
                                    ...
                                    @if($service->image)
                                        <img src="{{ asset('storage/' . $service->image) }}" class="w-40px h-40px rounded object-fit-cover" />
                                    @else
                                    ...
                                    @endif
                                    <span class="fw-bold text-gray-800 fs-6">{{ $service->title }}</span>
                                    <span class="badge badge-light-secondary fs-8">{{ $service->category ? $service->category->title : '-' }}</span>
                                </div>
                                <div class="d-flex align-items-center gap-2">
                                    <span class="badge badge-light-{{ $service->status == 'active' ? 'success' : 'danger' }} fw-bold">{{ $service->status == 'active' ? __('Aktif') : __('Pasif') }}</span>
                                    <span class="badge badge-light-primary fw-bold">#{{ $service->id }}</span>
                                </div>
                            </div>
                        @endforeach
                    </div>
...
<script>
    $(document).ready(function() {
        var el = document.getElementById('kt_services_sortable');
        if (el) {
            var sortable = Sortable.create(el, {
                ...
                onEnd: function (evt) {
                    var order = [];
                    $('#kt_services_sortable > div').each(function() {
                        order.push($(this).data('id'));
                    });
                    
                    $.ajax({
                        url: "{{ route($routePrefix . 'services.reorder.update') }}",
                        type: "POST",
                        data: {
                            _token: "{{ csrf_token() }}",
                            order: order
