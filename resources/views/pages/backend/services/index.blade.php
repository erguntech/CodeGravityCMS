...
<div class="page-title d-flex flex-column justify-content-center flex-wrap me-3 py-5">
    <h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center my-0">{{ __('Hizmetler') }}</h1>
...
<div id="kt_app_content" class="app-content flex-column-fluid">
    <div id="kt_app_content_container" class="app-container container-fluid">
            ...
            <div class="d-flex flex-column text-info">
                ...
                <span>- {{ __('Bu modül üzerinden işletmenizin gerçekleştirdiği, devam eden veya tamamlanan hizmetleri yönetebilirsiniz. Hizmetlere ait temel bilgiler, açıklamalar, görseller, kategoriler, sıralamalar ve yayın durumları bu modül üzerinden düzenlenebilir ve güncel tutulabilir.') }}</span>
            </div>
        </div>
<div class="card card-flush mb-5 mb-xl-10">
            <div class="card-header align-items-center gap-2 gap-md-5 position-relative ribbon ribbon-start min-h-60px">
                <div class="ribbon-label bg-primary fs-6" style="padding: 10px 15px;">
                    <span class="d-flex text-white fw-bolder fs-6">{{ __('Hizmetler Tablosu') }}</span>
                </div>
                ...
                <div class="card-toolbar">
                    <div class="d-flex align-items-center position-relative my-1 me-5">
                        ...
                        <input type="text" data-kt-user-table-filter="search" class="form-control form-control-sm form-control-solid w-250px ps-13" placeholder="{{ __('Hizmet Ara') }}" id="user-search-input" />
                    </div>
                    @if(auth()->user()->can('services.manage'))
                    @if(\App\Models\Service::count() > 1)
                    <a href="{{ route($routePrefix . 'services.reorder') }}" class="btn btn-icon btn-light-primary btn-sm me-3" title="{{ __('Hizmetleri Sırala') }}">
                        <i class="ki-duotone ki-dots-square fs-2">
                            <span class="path1"></span><span class="path2"></span><span class="path3"></span><span class="path4"></span>
                        </i>
                    </a>
                    @endif
                    <a href="{{ route($routePrefix . 'services.create') }}" class="btn btn-icon btn-light-success btn-sm me-3" title="{{ __('Hizmet Ekle') }}">
                        <i class="ki-duotone ki-plus fs-2"></i>
                    </a>
                    @else
                    @if(\App\Models\Service::count() > 1)
                    <a href="javascript:void(0)" class="btn btn-icon btn-light-primary btn-sm me-3 unauthorized-action-btn" title="{{ __('Hizmetleri Sırala') }}">
                        <i class="ki-duotone ki-dots-square fs-2">
                            <span class="path1"></span><span class="path2"></span><span class="path3"></span><span class="path4"></span>
                        </i>
                    </a>
                    @endif
                    <a href="javascript:void(0)" class="btn btn-icon btn-light-success btn-sm me-3 unauthorized-action-btn" title="{{ __('Hizmet Ekle') }}">
                        <i class="ki-duotone ki-plus fs-2"></i>
                    </a>
                    @endif
                    <button type="button" class="btn btn-icon btn-light-warning btn-sm" id="kt_services_table_reload" title="{{ __('Tabloyu Yenile') }}">
            ...
            <div class="card-body pt-6 pb-4">
                <table class="table align-middle table-row-dashed fs-6 gy-2" id="kt_services_table">
                    <thead>
                        <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                            <th class="min-w-50px">REF NO</th>
                            <th class="min-w-125px">{{ __('Hizmet Başlığı') }}</th>
...
<script>
    $(document).ready(function() {
        var table = $('#kt_services_table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route($routePrefix . 'services.index') }}",
            lengthChange: true,
            order: [[0, 'desc']],
            columns: [
        ...
        $('#kt_services_table_reload').on('click', function() {
            table.ajax.reload();
        });
        ...
        $(document).on('click', '.delete-record', function() {
            var id = $(this).data('id');
            var url = "{{ route($routePrefix . 'services.destroy', ':id') }}";
