...
@section('title', __('Müşteri Ekleme'))
...
<div class="page-title d-flex flex-column justify-content-center flex-wrap me-3 py-5">
    <h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center my-0">{{ __('Müşteri Ekleme') }}</h1>
    ...
    <div id="kt_app_content_container" class="app-container container-fluid">
        <div class="card card-flush mb-5 mb-xl-10">
            ...
            <div class="card-body pt-6">
                <form action="{{ route($routePrefix . 'clients.store') }}" method="POST" id="kt_user_create_form" autocomplete="off" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-6">
                            ...
                            <input type="text" name="company_name" class="form-control form-control-solid @error('company_name') is-invalid border-danger @enderror" placeholder="{{ __('İşletme Adı Giriniz') }}" value="{{ old('company_name') }}" />
                            @error('company_name')
                                <div class="text-danger small mt-1">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-12 mb-6">
                            ...
                            <select name="languages[]" class="form-select form-select-solid @error('languages') border-danger @enderror" data-control="select2" data-placeholder="{{ __('Seçim Yapınız') }}" multiple="multiple">
                                @foreach(config('languages') as $code => $lang)
                                    <option value="{{ $code }}" data-icon="{{ asset($lang['icon']) }}" {{ (collect(old('languages'))->contains($code)) ? 'selected' : '' }}>{{ $lang['name'] }}</option>
                                @endforeach
                            </select>
                            @error('languages')
                                <div class="text-danger small mt-1">{{ $message }}</div>
                    ...
                    <div class="d-flex justify-content-start">
                        <button type="submit" id="kt_user_create_submit" class="btn btn-sm btn-success me-3">
                            <span class="indicator-label">Müşteri Ekle</span>
