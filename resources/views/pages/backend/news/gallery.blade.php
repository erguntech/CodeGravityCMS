...
@section('title', __('Haber Galerisi Resimleri'))
...
<div class="page-title d-flex flex-column justify-content-center flex-wrap me-3 py-5">
    <h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center my-0">{{ __('Haber Galerisi Resimleri') }}</h1>
...
<div id="kt_app_content" class="app-content flex-column-fluid">
    <div id="kt_app_content_container" class="app-container container-fluid">
        ...
        @php
            ...
            $sizeStr = ($client && isset($client->image_sizes['news_gallery'])) ? $client->image_sizes['news_gallery'] . ' px' : null;
        @endphp
        ...
        <div class="card card-flush mb-4">
            <div class="card-header align-items-center gap-2 gap-md-5 position-relative ribbon ribbon-start min-h-60px">
                <div class="ribbon-label bg-warning fs-6" style="padding: 10px 15px;">
                    <span class="d-flex text-white fw-bolder fs-6">{{ __('Haber Galerisi Resimleri') }}</span>
                </div>
            ...
            <div class="card-body pt-6">
                ...
                <!-- Upload Form -->
                <form action="{{ route($routePrefix . 'news.gallery.store', $news->id) }}" method="POST" id="kt_news_gallery_upload_form" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-0">
                    ...
                    <div class="d-flex justify-content-start">
                        <button type="submit" id="kt_news_gallery_upload_submit" class="btn btn-sm btn-warning me-3">
                        ...
                        <div class="card shadow-sm border border-gray-300 rounded overflow-hidden position-relative">
                            ...
                            <div class="card-body p-2 d-flex justify-content-between align-items-center bg-light">
                                <div class="d-flex align-items-center gap-1">
                                    ...
                                    <a data-fslightbox="news-gallery" href="{{ asset('storage/' . $item->image) }}" class="btn btn-icon btn-sm btn-active-color-success" title="{{ __('Önizleme') }}">
...
<script>
    $(document).ready(function() {
        ...
        const form = document.getElementById('kt_news_gallery_upload_form');
        const submitButton = document.getElementById('kt_news_gallery_upload_submit');
        ...
        if (el) {
            var sortable = Sortable.create(el, {
                ...
                onEnd: function (evt) {
                    ...
                    $.ajax({
                        url: "{{ route($routePrefix . 'news.gallery.reorder', $news->id) }}",
                        type: "POST",
                        data: {
                            _token: "{{ csrf_token() }}",
                            order: order
        ...
        $(document).on('click', '.delete-gallery-item', function() {
            ...
            var url = "{{ route($routePrefix . 'news.gallery.destroy', [$news->id, ':id']) }}";
