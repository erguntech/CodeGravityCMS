...
@section('title', __('Medya Galerisi Resimleri'))
...
<div class="page-title d-flex flex-column justify-content-center flex-wrap me-3 py-5">
    <h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center my-0">{{ __('Medya Galerisi Resimleri') }}</h1>
    ...
    <div id="kt_app_content_container" class="app-container container-fluid">
        ...
        <div class="alert alert-dismissible bg-light-warning border border-warning border-dashed d-flex flex-column flex-sm-row p-5 mb-4">
            ...
            <div class="d-flex flex-column text-warning">
                ...
                <span>- Medya galerisine yüklediğiniz resimlerde <strong>herhangi bir otomatik boyutlandırma/kesme işlemi (crop) uygulanmaz.</strong></span>
                <span>- Dikey veya yatay tüm resimleriniz, web sitenizin ön yüzünde "Metro" mimarisinde (masonry) özgün halleriyle otomatik dizilecektir.</span>
        ...
        <div class="card card-flush mb-4">
            <div class="card-header align-items-center gap-2 gap-md-5 position-relative ribbon ribbon-start min-h-60px">
                <div class="ribbon-label bg-warning fs-6" style="padding: 10px 15px;">
                    <span class="d-flex text-white fw-bolder fs-6">{{ __('Medya Galerisi Resimleri') }}</span>
                </div>
            ...
            <div class="card-body pt-6">
                ...
                <!-- Upload Form -->
                <form action="{{ route($routePrefix . 'media.gallery.store', $media->id) }}" method="POST" id="kt_media_gallery_upload_form" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-0">
                    ...
                    <div class="d-flex justify-content-start">
                        <button type="submit" id="kt_media_gallery_upload_submit" class="btn btn-sm btn-warning me-3">
                        ...
                        <div class="card shadow-sm border border-gray-300 rounded overflow-hidden position-relative">
                            ...
                            <div class="card-body p-2 d-flex justify-content-between align-items-center bg-light">
                                <div class="d-flex align-items-center gap-1">
                                    ...
                                    <a data-fslightbox="media-gallery" href="{{ asset('storage/' . $item->image) }}" class="btn btn-icon btn-sm btn-active-color-success" title="{{ __('Önizleme') }}">
...
<script>
    $(document).ready(function() {
        ...
        const form = document.getElementById('kt_media_gallery_upload_form');
        const submitButton = document.getElementById('kt_media_gallery_upload_submit');
        ...
        if (el) {
            var sortable = Sortable.create(el, {
                ...
                onEnd: function (evt) {
                    ...
                    $.ajax({
                        url: "{{ route($routePrefix . 'media.gallery.reorder', $media->id) }}",
                        type: "POST",
                        data: {
                            _token: "{{ csrf_token() }}",
                            order: order
        ...
        $(document).on('click', '.delete-gallery-item', function() {
            ...
            var url = "{{ route($routePrefix . 'media.gallery.destroy', [$media->id, ':id']) }}";
