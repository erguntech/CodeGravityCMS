...
@section('title', __('Medyaları Sırala'))
...
<div class="page-title d-flex flex-column justify-content-center flex-wrap me-3 py-5">
    <h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center my-0">{{ __('Medyaları Sırala') }}</h1>
        ...
        @if(!$media->isEmpty())
            ...
            <div class="alert alert-dismissible bg-light-warning border border-warning border-dashed d-flex flex-column flex-sm-row p-5 mb-4">
                ...
                <div class="d-flex flex-column text-warning">
                    ...
                    <span>- Medya galerilerinin sol tarafındaki taşıma ikonunu (handle) kullanarak sürükle-bırak yöntemiyle sıralamalarını değiştirebilirsiniz.</span>
        ...
        <div class="card card-flush mb-5 mb-xl-10">
            <div class="card-header align-items-center gap-2 gap-md-5 position-relative ribbon ribbon-start min-h-60px">
                <div class="ribbon-label bg-warning fs-6" style="padding: 10px 15px;">
                    <span class="d-flex text-white fw-bolder fs-6">{{ __('Medya Sıralama Bilgileri') }}</span>
                </div>
                <div class="card-title"></div>
                <div class="card-toolbar">
                    <a href="{{ route($routePrefix . 'media.index') }}" class="btn btn-sm btn-light btn-active-light-primary" style="border-radius: 4px;">{{ __('Geri Dön') }}</a>
                </div>
            </div>
                ...
                @if($media->isEmpty())
                    ...
                    <div class="alert alert-dismissible bg-light-danger border border-danger border-dashed d-flex flex-column flex-sm-row p-5 mb-0">
                        ...
                        <div class="d-flex flex-column text-danger">
                            ...
                            <span>- Sıralanacak herhangi bir medya galerisi bulunamadı.</span>
                        </div>
                    </div>
                @else

                    <!-- Sortable Vertical List -->
                    <div class="d-flex flex-column gap-3 mb-0" id="kt_media_sortable">
...
<script>
    $(document).ready(function() {
        var el = document.getElementById('kt_media_sortable');
        if (el) {
            var sortable = Sortable.create(el, {
                ...
                onEnd: function (evt) {
                    var order = [];
                    $('#kt_media_sortable > div').each(function() {
                        order.push($(this).data('id'));
                    });
                    
                    $.ajax({
                        url: "{{ route($routePrefix . 'media.reorder.update') }}",
                        type: "POST",
                        data: {
                            _token: "{{ csrf_token() }}",
                            order: order
