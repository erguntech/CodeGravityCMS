...
@section('title', __('Projeleri Sırala'))
...
<div class="page-title d-flex flex-column justify-content-center flex-wrap me-3 py-5">
    <h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center my-0">{{ __('Projeleri Sırala') }}</h1>
        ...
        @if(!$projects->isEmpty())
            ...
            <div class="alert alert-dismissible bg-light-warning border border-warning border-dashed d-flex flex-column flex-sm-row p-5 mb-4">
                ...
                <div class="d-flex flex-column text-warning">
                    ...
                    <span>- Projelerin sol tarafındaki taşıma ikonunu (handle) kullanarak sürükle-bırak yöntemiyle sıralamalarını değiştirebilirsiniz.</span>
        ...
        <div class="card card-flush mb-5 mb-xl-10">
            <div class="card-header align-items-center gap-2 gap-md-5 position-relative ribbon ribbon-start min-h-60px">
                <div class="ribbon-label bg-warning fs-6" style="padding: 10px 15px;">
                    <span class="d-flex text-white fw-bolder fs-6">{{ __('Proje Sıralama Bilgileri') }}</span>
                </div>
                <div class="card-title"></div>
                <div class="card-toolbar">
                    <a href="{{ route($routePrefix . 'projects.index') }}" class="btn btn-sm btn-light btn-active-light-primary" style="border-radius: 4px;">{{ __('Geri Dön') }}</a>
                </div>
            </div>
                ...
                @if($projects->isEmpty())
                    ...
                    <div class="alert alert-dismissible bg-light-danger border border-danger border-dashed d-flex flex-column flex-sm-row p-5 mb-0">
                        ...
                        <div class="d-flex flex-column text-danger">
                            ...
                            <span>- Sıralanacak herhangi bir proje bulunamadı.</span>
                        </div>
                    </div>
                @else

                    <!-- Sortable Vertical List -->
                    <div class="d-flex flex-column gap-3 mb-0" id="kt_projects_sortable">
                        @foreach($projects as $project)
                            <div class="d-flex align-items-center justify-content-between border border-gray-300 rounded p-4 bg-light" data-id="{{ $project->id }}">
                                <div class="d-flex align-items-center gap-4">
                                    ...
                                    @if($project->image)
                                        <img src="{{ asset('storage/' . $project->image) }}" class="w-40px h-40px rounded object-fit-cover" />
                                    @else
                                    ...
                                    @endif
                                    <span class="fw-bold text-gray-800 fs-6">{{ $project->title }}</span>
                                    <span class="badge badge-light-secondary fs-8">{{ $project->category ? $project->category->title : '-' }}</span>
                                </div>
                                <div class="d-flex align-items-center gap-2">
                                    <span class="badge badge-light-{{ $project->status == 'active' ? 'success' : 'danger' }} fw-bold">{{ $project->status == 'active' ? __('Aktif') : __('Pasif') }}</span>
                                    <span class="badge badge-light-primary fw-bold">#{{ $project->id }}</span>
                                </div>
                            </div>
                        @endforeach
                    </div>
...
<script>
    $(document).ready(function() {
        var el = document.getElementById('kt_projects_sortable');
        if (el) {
            var sortable = Sortable.create(el, {
                ...
                onEnd: function (evt) {
                    var order = [];
                    $('#kt_projects_sortable > div').each(function() {
                        order.push($(this).data('id'));
                    });
                    
                    $.ajax({
                        url: "{{ route($routePrefix . 'projects.reorder.update') }}",
                        type: "POST",
                        data: {
                            _token: "{{ csrf_token() }}",
                            order: order
