...
@section('title', __('Güncelleme Düzenleme'))
...
<div class="page-title d-flex flex-column justify-content-center flex-wrap me-3 py-5">
    <h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center my-0">{{ __('Güncelleme Düzenleme') }}</h1>
    ...
    <div id="kt_app_content_container" class="app-container container-fluid">
        ...
        <div class="card card-flush mb-5 mb-xl-10">
            ...
            <div class="card-body pt-6">
                <form action="{{ route('admin.updates.update', $update->id) }}" method="POST" id="kt_updates_edit_form">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="col-md-12 mb-6">
                            <label class="form-label required">{{ __('Güncelleme Sürümü') }}</label>
                            <input type="text" name="version" class="form-control form-control-solid @error('version') border-danger @enderror" placeholder="{{ __('Örn: v1.0.0') }}" value="{{ old('version', $update->version) }}" />
                            @error('version')
                        ...
                        <div class="col-md-12 mb-6">
                            ...
                            <textarea name="notes" class="form-control form-control-solid @error('notes') border-danger @enderror" placeholder="{{ __('Güncelleme detaylarını buraya yazınız...') }}" rows="6">{{ old('notes', $update->notes) }}</textarea>
                            @error('notes')
                        ...
                        <div class="col-md-12 mb-0">
                            ...
                            <select name="status" class="form-select form-select-solid @error('status') border-danger @enderror" data-control="select2" data-hide-search="true" data-placeholder="{{ __('Seçim Yapınız') }}">
                                <option value=""></option>
                                <option value="active" {{ old('status', $update->status) == 'active' ? 'selected' : '' }}>{{ __('Aktif') }}</option>
                                <option value="passive" {{ old('status', $update->status) == 'passive' ? 'selected' : '' }}>{{ __('Pasif') }}</option>
                            </select>
                            @error('status')
                    ...
                    <div class="d-flex justify-content-start">
                        <button type="submit" id="kt_updates_edit_submit" class="btn btn-sm btn-warning me-3">
...
@push('scripts')
<script>
    const form = document.getElementById('kt_updates_edit_form');
    const submitButton = document.getElementById('kt_updates_edit_submit');
