...
@section('title', __('Güncelleme Yönetimi'))

@section('page_title')
<div class="page-title d-flex flex-column justify-content-center flex-wrap me-3 py-5">
    <h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center my-0">{{ __('Güncelleme Yönetimi') }}</h1>
    ...
    <div id="kt_app_content_container" class="app-container container-fluid">
        <div class="alert alert-dismissible bg-light-info border border-info border-dashed d-flex flex-column flex-sm-row p-5 mb-5">
            ...
            <div class="d-flex flex-column text-info">
                ...
                <span>- {{ __('Bu sayfada sisteme ait güncelleme notlarını görüntüleyebilir ve yönetebilirsiniz.') }}</span>
            </div>
        </div>
        <div class="card card-flush mb-5 mb-xl-10">
            <div class="card-header align-items-center gap-2 gap-md-5 position-relative ribbon ribbon-start min-h-60px">
                <div class="ribbon-label bg-primary fs-6" style="padding: 10px 15px;">
                    <span class="d-flex text-white fw-bolder fs-6">{{ __('Güncelleme Yönetimi Tablosu') }}</span>
                </div>
                ...
                <div class="card-toolbar">
                    <div class="d-flex align-items-center position-relative my-1 me-5">
                        ...
                        <input type="text" data-kt-user-table-filter="search" class="form-control form-control-sm form-control-solid w-250px ps-13" placeholder="{{ __('Güncelleme Ara') }}" id="user-search-input" />
                    </div>
                    @if(auth()->user()->can('updates.manage'))
                    <a href="{{ route('admin.updates.create') }}" class="btn btn-icon btn-light-success btn-sm me-3" title="{{ __('Güncelleme Ekle') }}">
                        <i class="ki-duotone ki-plus fs-2"></i>
                    </a>
                    @else
                    <a href="javascript:void(0)" class="btn btn-icon btn-light-success btn-sm me-3 unauthorized-action-btn" title="{{ __('Güncelleme Ekle') }}">
                        <i class="ki-duotone ki-plus fs-2"></i>
                    </a>
                    @endif
                    <button type="button" class="btn btn-icon btn-light-warning btn-sm" id="kt_updates_table_reload" title="{{ __('Tabloyu Yenile') }}">
            ...
            <div class="card-body pt-6 pb-4">
                <table class="table align-middle table-row-dashed fs-6 gy-2" id="kt_updates_table">
                    <thead>
                        <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                            <th class="min-w-50px">REF NO</th>
                            <th class="min-w-125px">{{ __('Sürüm') }}</th>
...
<script>
    $(document).ready(function() {
        var table = $('#kt_updates_table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('admin.updates.index') }}",
            lengthChange: true,
            order: [[0, 'desc']],
            columns: [
        ...
        $('#kt_updates_table_reload').on('click', function() {
            table.ajax.reload();
        });
        ...
        $(document).on('click', '.delete-record', function() {
            ...
            if (typeof SwalHelper !== 'undefined') {
                ...
                Swal.fire({
                    ...
                    text: '{{ __("Bu kaydı silmek istediğinize emin misiniz?") }}',
                    icon: "warning",
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                if(response.status === 'success') {
                                    ...
                                    Swal.fire({ text: response.message, icon: "success", buttonsStyling: false, confirmButtonText: '{{ __("Tamam") }}', customClass: { confirmButton: "btn btn-primary" } });
                                } else {
                                    Swal.fire({ text: response.message, icon: "error", buttonsStyling: false, confirmButtonText: '{{ __("Tamam") }}', customClass: { confirmButton: "btn btn-primary" } });
                                }
                            }
                        });
                    }
        ...
        $(document).on('click', '.unauthorized-action-btn', function() {
            Swal.fire({ text: '{{ __("Bu işlem için yetkiniz bulunmamaktadır.") }}', icon: "error", buttonsStyling: false, confirmButtonText: '{{ __("Tamam") }}', customClass: { confirmButton: "btn btn-primary" } });
        });
    });
</script>
@endpush
