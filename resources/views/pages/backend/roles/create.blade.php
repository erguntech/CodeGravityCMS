...
@section('title', __('Kullanıcı Rolü Ekleme'))
...
<div class="page-title d-flex flex-column justify-content-center flex-wrap me-3 py-5">
    <h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center my-0">{{ __('Kullanıcı Rolü Ekleme') }}</h1>
    ...
    <div id="kt_app_content_container" class="app-container container-fluid">
        <div class="card card-flush mb-5 mb-xl-10">
            ...
            <div class="card-body pt-6">
                <form action="{{ route($routePrefix . 'roles.store') }}" method="POST" id="kt_role_create_form" autocomplete="off">
                    @csrf
                    <div class="row mb-6">
                        <div class="col-md-12 mb-0">
                            <label class="form-label required">{{ __('Rol Adı') }}</label>
                            <input type="text" name="name" class="form-control form-control-solid @error('name') is-invalid border-danger @enderror" placeholder="Rol Adı Giriniz" value="{{ old('name') }}" />
                            @error('name')
                    ...
                    <div class="d-flex justify-content-start">
                        <button type="submit" id="kt_role_create_submit" class="btn btn-sm btn-success me-3">
                            <span class="indicator-label">{{ __('Kullanıcı Rolü Ekle') }}</span>
...
@push('scripts')
<script>
    const form = document.getElementById('kt_role_create_form');
    const submitButton = document.getElementById('kt_role_create_submit');
