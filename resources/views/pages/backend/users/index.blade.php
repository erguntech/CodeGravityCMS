...
<div class="page-title d-flex flex-column justify-content-center flex-wrap me-3 py-5">
    <h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center my-0">{{ __('Kullanıcılar') }}</h1>
        ...
        <div class="card card-flush mb-5 mb-xl-10">
            <div class="card-header align-items-center gap-2 gap-md-5 position-relative ribbon ribbon-start min-h-60px">
                <div class="ribbon-label bg-primary fs-6" style="padding: 10px 15px;">
                    <span class="d-flex text-white fw-bolder fs-6">Kullanıcılar Tablosu</span>
                </div>
                ...
                <div class="card-toolbar">
                    <div class="d-flex align-items-center position-relative my-1 me-5">
                        ...
                        <input type="text" data-kt-user-table-filter="search" class="form-control form-control-sm form-control-solid w-250px ps-13" placeholder="Kullanıcı Ara" id="user-search-input" />
                    </div>
                    @if(auth()->user()->can('users.manage'))
                    <a href="{{ route($routePrefix . 'users.create') }}" class="btn btn-icon btn-light-success btn-sm me-3" title="{{ __('Kullanıcı Ekle') }}">
                        <i class="ki-duotone ki-plus fs-2"></i>
                    </a>
                    @else
                    <a href="javascript:void(0)" class="btn btn-icon btn-light-success btn-sm me-3 unauthorized-action-btn" title="{{ __('Kullanıcı Ekle') }}">
                        <i class="ki-duotone ki-plus fs-2"></i>
                    </a>
                    @endif
                    <button type="button" class="btn btn-icon btn-light-warning btn-sm" id="kt_users_table_reload" title="{{ __('Tabloyu Yenile') }}">
            ...
            <div class="card-body pt-6 pb-4">
                <table class="table align-middle table-row-dashed fs-6 gy-2" id="kt_users_table">
                    <thead>
                        <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                            <th class="min-w-50px">REF NO</th>
                            <th class="min-w-125px">{{ __('Ad Soyad') }}</th>
                            <th class="min-w-125px">{{ __('E-posta') }}</th>
                            <th class="min-w-100px">{{ __('Kullanıcı Tipi') }}</th>
...
<script>
    $(document).ready(function() {
        var table = $('#kt_users_table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route($routePrefix . 'users.index') }}",
            lengthChange: true,
            columns: [
                ...
                {data: 'user_type', name: 'user_type', className: 'align-middle'},
        ...
        $('#kt_users_table_reload').on('click', function() {
            table.ajax.reload();
        });
        ...
        $(document).on('click', '.delete-user', function() {
            var id = $(this).data('id');
            var url = "{{ route($routePrefix . 'users.destroy', ':id') }}";
