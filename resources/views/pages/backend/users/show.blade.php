...
@section('title', __('Kullanıcı Detayları'))

@section('page_title')
<div class="page-title d-flex flex-column justify-content-center flex-wrap me-3 py-5">
    <h1 class="page-heading d-flex text-gray-900 fw-bold fs-3 flex-column justify-content-center my-0">{{ __('Kullanıcı Profili') }}</h1>
                    ...
                    <div class="card-body pt-15">
                        ...
                        <div class="d-flex flex-stack fs-4 py-3">
                            ...
                            <span data-bs-toggle="tooltip" data-bs-trigger="hover" title="{{ __('Düzenle') }}">
                                <a href="{{ route($routePrefix . 'users.edit', $user->id) }}" class="btn btn-sm btn-light-primary">{{ __('Düzenle') }}</a>
                            </span>
                        </div>
                        ...
                        <div class="card card-flush mb-6 mb-xl-9">
                            <div class="card-header mt-6">
                                <div class="card-title flex-column">
                                    <h2 class="mb-1">{{ __('Roller ve Yetkiler') }}</h2>
                                    <div class="fs-6 fw-semibold text-muted">{{ __('Kullanıcıya tanımlanmış roller ve özel yetkiler') }}</div>
                                </div>
                            </div>
                            <div class="card-body p-9 pt-4">
                                <div class="mb-10">
                                    <h5 class="mb-4">{{ __('Roller') }}:</h5>
                                    @foreach($user->roles as $role)
                                        <span class="badge badge-light-primary fs-7 fw-bold me-2">{{ $role->name }}</span>
                                    @endforeach
                                </div>
                                <div>
                                    <h5 class="mb-4">{{ __('Özel Yetkiler') }}:</h5>
                                    @foreach($user->permissions as $permission)
                                        <span class="badge badge-light-info fs-7 fw-bold me-2">{{ $permission->name }}</span>
                                    @endforeach
                                    @if($user->permissions->count() == 0)
                                        <span class="text-muted fs-7">{{ __('Özel yetki tanımlanmamış.') }}</span>
                                    @endif
                                </div>
                            </div>
                        </div>
