...
class ServiceCategoryController extends Controller implements HasMiddleware
{
    ...
    {
        return [
            new Middleware('can:service-categories.view', only: ['index', 'show']),
            new Middleware('can:service-categories.manage', except: ['index', 'show']),
        ];
    }
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = ServiceCategory::with('parent')->select(['id', 'title', 'parent_id', 'status', 'image', 'created_at', 'sort_order']);
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    $canManage = auth()->user()->can('service-categories.manage');
                    
                    $viewBtn = '';
                    if ($row->image) {
                        $viewBtn = '<a data-fslightbox="service-categories-gallery" data-type="image" href="' . asset('storage/' . $row->image) . '" class="btn btn-icon btn-light-primary btn-sm me-1" title="' . __('Resmi Görüntüle') . '">
                    ...
                    if ($canManage) {
                        $routePrefix = 'client.';
                        $editUrl = route($routePrefix . 'service-categories.edit', $row->id);
        ...
        return view('pages.backend.service_categories.index');
    }

    public function create()
    {
        $categories = ServiceCategory::whereNull('parent_id')->get();
        return view('pages.backend.service_categories.create', compact('categories'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'parent_id' => ['nullable', $this->existsForClient('service_categories')],
            ...
            'title' => __('Hizmet Kategorisi Başlığı'),
            'title.' . $clientDefaultLang => __('Hizmet Kategorisi Başlığı'),
            'description' => __('Hizmet Kategorisi Açıklaması'),
            'parent_id' => __('Üst Kategori'),
            'status' => __('Durumu'),
            'image' => __('Hizmet Kategorisi Manşet Resmi'),
        ...
        if ($request->hasFile('image')) {
            ...
            if ($client) {
                $size = $client->getImageSize('service_category');
                if ($size) {
        ...
        return redirect()->route('client.service-categories.index')->with('success', __('Kayıt başarı ile sisteme eklendi.'));
    }

    public function edit(ServiceCategory $serviceCategory)
    {
        $categories = ServiceCategory::whereNull('parent_id')->where('id', '!=', $serviceCategory->id)->get();
        return view('pages.backend.service_categories.edit', compact('serviceCategory', 'categories'));
    }
    ...
    public function update(Request $request, ServiceCategory $serviceCategory)
    {
        ...
        if ($request->title !== $serviceCategory->title) {
            $titles = $request->input('title');
        $firstTitle = is_array($titles) ? reset($titles) : $titles;
        $data['slug'] = Str::slug($firstTitle);
        }
        ...
        return redirect()->route('client.service-categories.index')->with('success', __('Kayıt başarı ile güncellendi.'));
    }
    ...
    public function reorder()
    {
        $categories = ServiceCategory::whereNull('parent_id')
            ...
            ->get();
        return view('pages.backend.service_categories.reorder', compact('categories'));
    }
