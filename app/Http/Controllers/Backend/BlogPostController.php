...
use App\Models\BlogPostGallery;
...
class BlogPostController extends Controller implements HasMiddleware
{
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = BlogPost::with('category')->select(['id', 'blog_post_category_id', 'title', 'status', 'image', 'created_at', 'sort_order']);
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $viewBtn = '';
                    if ($row->image) {
                        $viewBtn = '<a data-fslightbox="blog_posts-gallery" data-type="image" href="' . asset('storage/' . $row->image) . '" class="btn btn-icon btn-light-primary btn-sm me-1" title="' . __('Resmi Görüntüle') . '">
                    ...
                    if ($canManage) {
                        $routePrefix = 'client.';
                        $editUrl = route($routePrefix . 'blog-posts.edit', $row->id);
                    ...
                    $galleryBtn = '';
                    if ($canManage) {
                        $galleryUrl = route('client.blog-posts.gallery', $row->id);
        ...
        return view('pages.backend.blog_posts.index');
    }

    public function create()
    {
        $categories = BlogPostCategory::all();
        return view('pages.backend.blog_posts.create', compact('categories'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'title' => __('Blog Yazısı Başlığı'),
            'title.' . $clientDefaultLang => __('Blog Yazısı Başlığı'),
            'blog_post_category_id' => __('Kategori'),
            'description' => __('Blog Yazısı Açıklaması'),
            'status' => __('Durumu'),
            'image' => __('Blog Yazısı Manşet Resmi'),
        ...
        if ($request->hasFile('image')) {
            $data['image'] = $request->file('image')->store('blog_posts', 'public');

            $client = auth()->user()->client;
            if ($client) {
                $size = $client->getImageSize('blog_post');
                if ($size) {
        ...
        $data['sort_order'] = BlogPost::max('sort_order') + 1;
        BlogPost::create($data);

        return redirect()->route('client.blog-posts.index')->with('success', __('Kayıt başarı ile sisteme eklendi.'));
    }

    public function edit(BlogPost $blog_post)
    {
        $categories = BlogPostCategory::all();
        return view('pages.backend.blog_posts.edit', compact('blog_post', 'categories'));
    }
    ...
    public function update(Request $request, BlogPost $blog_post)
    {
        ...
        return redirect()->route('client.blog-posts.index')->with('success', __('Kayıt başarı ile güncellendi.'));
    }
    ...
    public function destroy(BlogPost $blog_post)
    {
        try {
            ...
            $blog_post->delete();
            return response()->json(['status' => 'success', 'message' => __('Blog Yazısı başarıyla silindi.')]);
        } catch (\Exception $e) {
            return response()->json(['status' => 'error', 'message' => __('Bir hata oluştu!')]);
        }
    }

    public function gallery(BlogPost $blog_post)
    {
        $gallery = $blog_post->gallery;
        return view('pages.backend.blog_posts.gallery', compact('blog_post', 'gallery'));
    }

    public function storeGallery(Request $request, BlogPost $blog_post)
    {
        ...
        $lastSortOrder = $blog_post->gallery()->max('sort_order') ?? 0;
        foreach ($request->file('images') as $file) {
            $lastSortOrder++;
            $path = $file->store('blog_posts/gallery', 'public');

            $client = auth()->user()->client;
            if ($client) {
                $size = $client->getImageSize('blog_post_gallery');
                if ($size) {
                    \App\Helpers\ImageHelper::resizeAndCrop(Storage::disk('public')->path($path), $size['width'], $size['height']);
                }
            }

            BlogPostGallery::create([
    ...
    public function reorderGallery(Request $request, BlogPost $blog_post)
    {
        $clientDefaultLang = auth()->user()->client?->languages()->where('is_default', true)->first()?->code ?? 'tr';
        $request->validate([
    ...
    {
        $blog_posts = BlogPost::orderBy('sort_order', 'asc')->get();
        return view('pages.backend.blog_posts.reorder', compact('blog_posts'));
    }
    ...
    public function updateOrder(Request $request)
    {
        ...
        $order = $request->input('order');
        foreach ($order as $index => $id) {
            BlogPost::where('id', $id)->update(['sort_order' => $index + 1]);
        }
