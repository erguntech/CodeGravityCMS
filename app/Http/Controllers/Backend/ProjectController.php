...
class ProjectController extends Controller implements HasMiddleware
{
    public static function middleware(): array
    {
        return [
            ...
            new Middleware('can:projects.manage', except: ['index', 'show']),
        ];
    }
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = Project::with('category')->select(['id', 'project_category_id', 'title', 'status', 'image', 'created_at', 'sort_order']);
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    $canManage = auth()->user()->can('projects.manage');
                    
                    $viewBtn = '';
                    if ($row->image) {
                        $viewBtn = '<a data-fslightbox="projects-gallery" data-type="image" href="' . asset('storage/' . $row->image) . '" class="btn btn-icon btn-light-primary btn-sm me-1" title="' . __('Resmi Görüntüle') . '">
                    ...
                    if ($canManage) {
                        $routePrefix = 'client.';
                        $editUrl = route($routePrefix . 'projects.edit', $row->id);
                    ...
                    $galleryBtn = '';
                    if ($canManage) {
                        $galleryUrl = route('client.projects.gallery', $row->id);
    ...
    public function create()
    {
        ...
        return view('pages.backend.projects.create', compact('categories'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'title' => __('Proje Başlığı'),
            'title.' . $clientDefaultLang => __('Proje Başlığı'),
            'project_category_id' => __('Kategori'),
            'description' => __('Proje Açıklaması'),
            'status' => __('Durumu'),
            'image' => __('Proje Manşet Resmi'),
        ...
        if ($request->hasFile('image')) {
            ...
            if ($client) {
                $size = $client->getImageSize('project');
                if ($size) {
        ...
        return redirect()->route('client.projects.index')->with('success', __('Kayıt başarı ile sisteme eklendi.'));
    }

    public function edit(Project $project)
    {
        $categories = ProjectCategory::all();
        return view('pages.backend.projects.edit', compact('project', 'categories'));
    }
    ...
    public function update(Request $request, Project $project)
    {
        ...
        return redirect()->route('client.projects.index')->with('success', __('Kayıt başarı ile güncellendi.'));
    }
    ...
    public function destroy(Project $project)
    {
        try {
            ...
            $project->delete();
            return response()->json(['status' => 'success', 'message' => __('Proje başarıyla silindi.')]);
        } catch (\Exception $e) {
    ...
    public function gallery(Project $project)
    {
        ...
        return view('pages.backend.projects.gallery', compact('project', 'gallery'));
    }
    ...
    public function storeGallery(Request $request, Project $project)
    {
        ...
        $lastSortOrder = $project->gallery()->max('sort_order') ?? 0;
        foreach ($request->file('images') as $file) {
            ...
            $client = auth()->user()->client;
            if ($client) {
                $size = $client->getImageSize('project_gallery');
                if ($size) {
    ...
    public function reorder()
    {
        ...
        return view('pages.backend.projects.reorder', compact('projects'));
    }
